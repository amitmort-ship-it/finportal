...
import { FileSignature, Save, ExternalLink } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Label } from '@/components/ui/label';
import { toast } from 'sonner';

export default function AdminServiceAgreement({ selectedClient }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = await base44.entities.ServiceAgreement.filter({ client_email: selectedClient });
        const existing = data?.[0] || null;
        setRecord(existing);
        setSignUrl(existing?.sign_url || '');
      } catch {
        toast.error('שגיאה בטעינת הסכם ליווי');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      if (record) {
        const updated = await base44.entities.ServiceAgreement.update(record.id, { sign_url: signUrl });
        setRecord(updated);
      } else {
        const created = await base44.entities.ServiceAgreement.create({ client_email: selectedClient, sign_url: signUrl });
        setRecord(created);
      }
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-bold">הסכם ליווי לחתימה</h3>
      </div>
      <div>
